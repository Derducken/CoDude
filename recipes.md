...
**📷 Suggest Favicons**: Analyze this text and suggest at least three favicons that reflect the content. If it's a list, suggest three favicons for each point. Start your answer with a copy of the text that includes those favicons - if it's a list, adding the favicon at the very beginning of each list entry, followed by a space. Justify your suggestions. The text is:

**🔗 Suggest Links**: USETOOLS: You are an article enrichment assistant. Follow these steps exactly:\n1. Analyze the input text: Summarize key themes and identify 3-5 terms/phrases that readers might want more info on (e.g., technical terms, historical references).\n2. For each term, use the 'full_web_search' tool to find 1-2 reliable sources. Query must be precise (e.g., "definition and explanation of [term]"). Perform an individual search for each term - so, if the user asks for "best emulators for genesis, snes, and neo-geo", perform a full_web_search for "best emulators for genesis", then a full_web_search for "best emulators for snes", then another full_web_search for "best emulators for neo-geo".\n3. From the tool results, extract real URLs, snippets (short quotes or summaries), and ensure they are accurate—do not invent any data.\n4. Output a list of anchors in this format:\n - Term: [term]\n - Snippet: [brief excerpt from source]\n - URL: [actual full URL]\nYou MUST think step-by-step and use the full-web-search tool before responding, calling the tool at least once per term, and ONLY using data from the tool outputs. If no relevant results, state you didn't find reliable sources instead of guessing, but DO list the terms you searched for.\nThe text is:

**🔗 Suggest Links - WIKIPEDIA**: USETOOLS: You are an article enrichment assistant. Follow these steps exactly:\n1. Analyze the input text: Summarize key themes and identify 3-5 terms/phrases that readers might want more info on (e.g., technical terms, historical references).\n2. For each term, use the 'full_web_search' tool to search for those terms on WikiPedia. To achieve this, end each of your search queries with "+wikipedia" - for example, for the term "formatted SD card", search for "formatted SD card +wikipedia". Query must be precise (e.g., "definition and explanation of [term]"). Perform an individual search for each term - so, if the user asks for "best emulators for genesis, snes, and neo-geo", perform a full_web_search for "best emulators for genesis", then a full_web_search for "best emulators for snes", then another full_web_search for "best emulators for neo-geo".\n3. From the tool results, extract real Wikipedia URLs, snippets (short quotes or summaries), and ensure they are accurate—do not invent any data.\n4. Output a list of anchors in this format:\n - Term: [term]\n - Snippet: [brief excerpt from source]\n - URL: [actual full URL]\nYou MUST think step-by-step and use the full-web-search tool before responding, calling the tool at least once per term, and ONLY using data from the tool outputs. If no relevant results, state you didn't find reliable sources instead of guessing, but DO list the terms you searched for.\nThe text is:

**🔗 Suggest Links - REDDIT**: USETOOLS: You are an article enrichment assistant. Follow these steps exactly:\n1. Analyze the input text: Summarize key themes and identify 3-5 terms/phrases that readers might want more info on (e.g., technical terms, historical references).\n2. For each term, use the 'full_web_search' tool to search for those terms on Reddit. To achieve this, end each of your search queries with "+reddit" - for example, for the term "formatted SD card", search for "formatted SD card +reddit". Query must be precise (e.g., "definition and explanation of [term]"). Perform an individual search for each term - so, if the user asks for "best emulators for genesis, snes, and neo-geo", perform a full_web_search for "best emulators for genesis", then a full_web_search for "best emulators for snes", then another full_web_search for "best emulators for neo-geo".\n3. From the tool results, extract real Reddit URLs, snippets (short quotes or summaries), and ensure they are accurate—do not invent any data.\n4. Output a list of anchors in this format:\n - Term: [term]\n - Snippet: [brief excerpt from source]\n - URL: [actual full URL]\nYou MUST think step-by-step and use the full-web-search tool before responding, calling the tool at least once per term, and ONLY using data from the tool outputs. If no relevant results, state you didn't find reliable sources instead of guessing, but DO list the terms you searched for.\nThe text is:
...
**📝 Make Note+**: Analyze the following content thoroughly. Extract the highlights, core concepts, key points, and most valuable snippets of information. Then, create a well-structured note in Markdown using the following format: Start with a metadata section enclosed between two lines containing only "---". Inside, write: "Tags: [ tag1, tag2 ]" (replace with relevant keywords based on the content), "Type: note", "Status:" (leave this one empty), "Source:" (always include this line, even if the source is unknown or not provided; leave it blank if no source is given), "Time: 23:59 2025-09-19" (replace with the current time and date in this format: HH:MM YYYY-MM-DD). Place each metadata entry on its own line. Do not omit any metadata fields. After the metadata, add one empty line. Then, write a clear and reflective title as an H1 heading (do not reuse the original title; rewrite to better reflect the note’s content). Write a short introductory sentence or two to summarize the essence of the content. Then include a section with an H2 heading summarizing the next list (choose an appropriate heading like “Key Strategies”, “Core Ideas”, “Why It Works”, “Main Takeaways”, etc., depending on the content type). Below that, provide a bullet list (maximum 5 points) capturing the core ideas or highlights. Optionally, add a second H2 heading if another major point or explanation is needed, followed by more text, another list, or both. Format the entire output in clean, professional Markdown. Your goal is to turn raw, varied content (e.g., web articles, Reddit threads, tutorials, reviews) into evergreen, digestible notes. The content is:
...
**🗜️ Summarize in 5 Bullet Points**: Summarize the following text in five short and easily digestible bullet points, presented as a list. Your answer should contain ONLY the bullet points, no intro, no outro, no other explanation or additional information. The text is:
